...
//SHOWING TIME REMAINING FOR THE PASSANGER FOR THE SPECIFIC SECTION
  ...
  $start = ($page-1) * $total;
	...
	{	
		...
		$query	=	"select * from lox_payments where (lox_user_id ='$universal_id' && lox_passanger_type =6  && lox_access_time_expiry=1)";
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="rn-page-title-inner">
							<h1><?php echo $customer_logistics_search ?></h1>
												...
												<div class="col-sm-6">
													<div class="form-group">
														<select name="city" class="form-control" >
																<option value="">-Select your location to find nearby customer logistics requests-</option>
...
<?php 						
				...
				if ($city!="" )
				{
					...
					$query	=	"SELECT  * FROM users WHERE status=1 and role_id=6 and cityname='".$city."'  ORDER BY date_updated DESC LIMIT $start,$total";
				}
				
				else
				{
					$query	= "SELECT  * FROM users WHERE status=1 and role_id=6  ORDER BY date_updated DESC LIMIT $start,$total";
				}
...
?>
    ...
    <div class="col-md-6 col-xl-4">                       
      <div class="card">
        <div class="card-body">
          <div class="media align-items-center"><span style="background-color:white " class="avatar avatar-xl mr-3"></span>
            <div class="media-body overflow-hidden">
              ...
              <p class="card-text">
                 <?php if($ok==1){?>		<span><strong>Pickup Date:</strong><?php echo $row1['pickup_date']?></span><br>
											<span><strong>Email: </strong><?php echo $row1['email']; ?></span><br>
											<span><strong>Mobile: </strong><?php echo $row1['mobile']?></span><br>
											
										<?php }else{?>	
											<span><strong>Pickup Date: </strong><?php echo $row1['pickup_date']?></span>	<br>
										...
										<?php }else{?>
											<a class="btn btn-sm btn-shadow btn-success " target="_blank" href="details.php?id=<?php echo md5($row1["id"]);?>&&section=<?php echo md5(6)?>">Show Full Request Details</a>
											<?php }?>
					...
					<div class="col-lg-12">		 
						<nav aria-label="..." class="rn-pagination rn-pagination-center">
					        ...
					        if ($city!="" )
							{
								...
								$slt	=	"SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=6 and users.cityname='".$city."' ";
							}
							
							else
							{
								$slt	= "SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=6  ";
							}  
