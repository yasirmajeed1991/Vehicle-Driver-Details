...
if(empty($CompanyRef) && empty($PnrID) && !empty($TransID) && !empty($TransactionToken) && empty($CCDapproval))
{
    ...
    if($TransID==$TransactionToken)
    {
        ...
        //Fetching data on the basis of selection to put payment record in a lox_payments table
            ...
            {
                $query ="UPDATE lox_payments SET lox_payment_date='$current_date_time',lox_payment_status=1,lox_access_time_expiry=1,lox_comments='Transaction Failed' where lox_transaction_id='$TransactionToken' ";
            $rs = mysqli_query($conn,$query) or die(mysqli_error());
  ...
  <head>
    ...
    <body>
      ...
      <div style="border-radius:200px; height:200px; width:200px; background: #F8FAF5; margin:0 auto;">
        ...
        <h1>Oops!</h1> 
        <p>There is a problem in your transaction.</p>
        <p></p>
      </div>
      <script>
         setTimeout(function(){
