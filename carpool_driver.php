...
//SHOWING TIME REMAINING FOR THE PASSANGER FOR THE SPECIFIC SECTION
  ...
  $start = ($page-1) * $total;
	...
	{	
		...
		$query	=	"select * from lox_payments where (lox_user_id ='$universal_id' && lox_passanger_type =4  && lox_access_time_expiry=1)";
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="rn-page-title-inner">
							<h1><?php echo $carpool_driver_search?></h1>
												...
												<div class="col-sm-6">
													<div class="form-group">
														<select name="city" class="form-control" >
																<option value="">-Select your location to find nearby Car Pool Drivers-</option>
...
<?php 						
				...
				if ($city!=""  && $type=="")
				{
					...
					$query	=	"SELECT  * FROM users WHERE status=1 and role_id=4 and cityname='".$city."' ORDER BY RAND() LIMIT $start,$total";
				}
				elseif ($type!="" && $city=="" )
				{
					
					$query	=	"SELECT  * FROM users WHERE status=1 and role_id=4 and vehcile_type = '".$type."' ORDER BY RAND() LIMIT $start,$total";
				}
				elseif ($type!="" && $city!="" )
				{
					
					$query	=	"SELECT  * FROM users WHERE status=1 and role_id=4 and vehcile_type = '".$type."' and cityname='".$city."' ORDER BY RAND() LIMIT $start,$total";
				}
				else
				{
					$query	= "SELECT  * FROM users  WHERE status=1 and role_id=4 ORDER BY RAND() LIMIT $start,$total";
				}
...
?>								<!-- Car Search Item-->
								<div class="rn-car-search-item">
									<?php /*<div class="rn-car-search-item-thumb">
										<a>
											
    										<?php 
    										$url_img = "uploads/".$row1['picture']."";
    										if(file_exists($url_img))
    										{
    
    										?>
    										<img class="img-fluid" src="" alt="" srcset="uploads/<?php echo $row1['picture']?> 1x, uploads/<?php echo $row1['picture']?> 1x"/>
    										...
    										{
    										?>	
    										<img class="img-fluid" src="" alt="" srcset="uploads/noimageavl.jpg 1x, uploads/noimageavl.jpg 1x"/>
    									<?php }?>
									    </a>
									</div> */?>
...
<a href="#"><?php if($ok==1){echo  $row1['name'];}else{echo $dispnum = substr($row1['name'], 0, 4) . str_repeat("*", strlen($row1['name'])-2);}?></a>
									...
									<div class="rn-car-search-item-pricing">
										...
										<?php }else{?>
											<a class="btn btn-sm btn-shadow btn-success" href="details.php?id=<?php echo md5($row1["id"]);?>&&section=<?php echo md5(4)?>">Show Full Contact Details</a>
											<?php }?>
...
?>
					        ...
					        if ($city!=""  && $type=="")
				{
					...
					$slt	=	"SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=4 and users.cityname='".$city."' ";
				}
				elseif ($type!="" && $city=="" )
				{
					
					$slt	=	"SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=4 and users.vehcile_type = '".$type."' ";
				}
				elseif ($type!="" && $city!="" )
				{
					
					$slt	=	"SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=4 and users.vehcile_type = '".$type."' and users.cityname='".$city."' ";
				}
				else
				{
					$slt	= "SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=4 ";
				}    
