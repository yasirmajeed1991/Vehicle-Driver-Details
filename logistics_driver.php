...
//SHOWING TIME REMAINING FOR THE PASSANGER FOR THE SPECIFIC SECTION
  ...
  $start = ($page-1) * $total;
	...
	{	
		...
		$query	=	"select * from lox_payments where (lox_user_id ='$universal_id' && lox_passanger_type =3  && lox_access_time_expiry=1)";
				...
				<div class="row">
					<div class="col-lg-12">
						<div class="rn-page-title-inner">
							<h1><?php echo $logistic_driver ?></h1>
												...
												<div class="col-sm-6">
													<div class="form-group">
														<select name="city" class="form-control" >
																<option value="">-Select your location to find nearby logistics truck drivers-</option>
...
<?php 						
				...
				if ($city!="" )
				{
					...
					$query	=	"SELECT  * FROM users  WHERE status=1 and role_id=3 and cityname='".$city."' ORDER BY RAND() LIMIT $start,$total";
				}
				
				else
				{
					$query	= "SELECT  * FROM users WHERE status=1 and role_id=3 ORDER BY RAND() LIMIT $start,$total";
				}
...
<a href="#"><?php if($ok==1){echo  $row1['name'];}else{echo $dispnum = substr($row1['name'], 0, 4) . str_repeat("*", strlen($row1['name'])-2);}?></a>
									...
									<div class="rn-car-search-item-pricing">
										...
										<?php }else{?>
											<a class="btn btn-sm btn-shadow btn-success" href="details.php?id=<?php echo md5($row1["id"]);?>&&section=<?php echo md5(3)?>">Show Full Contact Details</a>
											<?php }?>
					...
					<div class="col-lg-12">		 
						<nav aria-label="..." class="rn-pagination rn-pagination-center">
					        ...
					        if ($city!="" )
							{
								...
								$slt	=	"SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=3 and users.cityname='".$city."' ";
							}
							
							else
							{
								$slt	= "SELECT  * FROM users INNER JOIN role ON role.user_id = users.id WHERE users.status=1 and role.role_id=3  ";
							}  
